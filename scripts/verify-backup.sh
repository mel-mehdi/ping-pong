...
if [ $? -eq 0 ]; then
    ...
    echo "Cleaning up..."
    docker exec $DB_CONTAINER psql -U $DB_USER -c "DROP DATABASE $TEST_DB_NAME;" 2>/dev/null
else
    echo "✗ Restore failed - backup might be corrupted"
    docker exec $DB_CONTAINER psql -U $DB_USER -c "DROP DATABASE IF EXISTS $TEST_DB_NAME;" 2>/dev/null
    rm -f /tmp/test_restore.sql
    exit 1
fi
