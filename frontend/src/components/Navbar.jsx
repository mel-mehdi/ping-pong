    ...
    try {
      ...
      setNotifications(
        (requests || [])
          ...
          .map((r) => ({
            id: r.id,
            fromName: r.fromName || r.from_user?.username || r.from_user || 'Unknown',
          }))
      );
    } catch (error) {
    ...
    if (query.trim().length >= 1) {
      ...
      try {
        ...
        const normalized = (results || [])
          .filter((u) => u && typeof u === 'object' && (u.username || u.email))
      ...
      <nav className="navbar" role="navigation" aria-label="Main navigation">
        <div className="nav-container">
          ...
          <ul className="nav-menu">
            ...
            <li>
              <Link to="/tournament" className={isActive('/tournament') ? 'active' : ''}>
                {t('nav.tournaments')}
              </Link>
            </li>
            <li>
          ...
          <div className="nav-actions">
            {isAuthenticated && (
              <div className="nav-search-wrapper">
                <div
                  className={`nav-search-input-wrapper ${searchExpanded ? 'search-expanded' : ''}`}
                >
                  <input
                    ref={searchInputRef}
                    type="text"
                      ...
                      <>
                        ...
                        >
                          {searchSource === 'backend'
                            ? t('search.backend_results')
                            : t('search.client_results')}
                        </div>
                        {searchResults
                          .filter(
                            (u) =>
                              ...
                              (!userData ||
                                (u.username !== userData.username &&
                                  (u.id || u.userId) !== (userData.userId || userData.id)))
                          )
                          .map((user) => (
                            <div key={user.id} className="nav-search-result-item">
                              <div className="nav-search-result-info">
                                ...
                                <div className="nav-user-details">
                                  ...
                                  <span
                                    className={`nav-user-status ${user.online_status ? 'online' : 'offline'}`}
                                  >
